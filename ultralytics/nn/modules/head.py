...
# 从 models/density_head.py 导入
try:
    from models.density_head import DensityGuidedQuerySelector
    print(f"✅ [W-RT-DETR] 密度引导头加载成功")
except ImportError as e:
    print(f"❌ [W-RT-DETR] 密度引导头加载失败: {e}")
# =======================================================
...
class RTDETRDecoder(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        # === 🟢 [W-RT-DETR] 初始化密度引导选择器 ===
        # hd: hidden_dim (256), nq: num_queries (300)
        self.density_selector = DensityGuidedQuerySelector(hidden_dim=hd, num_queries=nq)
        # =========================================
    ...
    def forward(self, x: list[torch.Tensor], batch: dict | None = None) -> tuple | torch.Tensor:
        ...
        # === 🟢 [W-RT-DETR 修复] 传入 raw_features=x ===
        # 原代码: embed, refer_bbox, enc_bboxes, enc_scores = self._get_decoder_input(feats, shapes, dn_embed, dn_bbox)
    ...
    def _get_decoder_input(
            ...
            raw_features: list[torch.Tensor] = None,  # 👈 新增参数
        ...
        # === 🟢 [W-RT-DETR] 核心植入: 密度引导 Query 选择 ===

        # 1. 还原 P3 特征 (高分辨率层)
        p3_h, p3_w = shapes[0]
        p3_len = p3_h * p3_w
        p3_feat_flat = features[:, :p3_len, :]
        p3_feat_2d = p3_feat_flat.transpose(1, 2).view(bs, self.hidden_dim, p3_h, p3_w)

        # 2. 预测密度图
        _, density_map = self.density_selector(p3_feat_2d)

        # 3. 构造评分向量
        density_scores = density_map.flatten(2).squeeze(1)
        total_len = enc_outputs_scores.shape[1]

        # 4. 填充 (只保留 P3 分数，其他设为 -inf)
        full_density_scores = torch.full((bs, total_len), -1e9, device=feats.device)
        valid_len = min(p3_len, density_scores.shape[1])
        full_density_scores[:, :valid_len] = density_scores[:, :valid_len]

        # 5. Top-K 选择
        topk_ind = torch.topk(full_density_scores, self.num_queries, dim=1).indices.view(-1)
        # ================================================
