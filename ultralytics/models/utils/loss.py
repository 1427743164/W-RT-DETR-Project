...
# =======================================================
# 🟢 [W-RT-DETR 自定义 Loss 植入]
# 1. 动态获取项目根目录
#    (loss.py 在 ultralytics/models/utils，往上跳 3 级是根目录)
...
# 3. 导入 NWD Loss
#    假设你已经把根目录下的 utils 改名为 project_utils (强烈推荐!)
#    如果你没改名，下面就写 from utils.losses import nwd_loss
try:
    from utils.losses import nwd_loss
    print(f"✅ [W-RT-DETR] 成功加载 NWD Loss: {project_root}")
except ImportError as e:
    print(f"❌ [W-RT-DETR] NWD Loss 加载失败: {e}")
# =======================================================
...
class DETRLoss(nn.Module):
    ...
    def _get_loss_bbox(
        ...
        # === 🟢 W-RT-DETR 修改开始 ===
        # 1. 原版 GIoU
        giou_val = 1.0 - bbox_iou(pred_bboxes, gt_bboxes, xywh=True, GIoU=True)

        # 2. 你的 NWD Loss (自动调用你项目里的代码)
        #    constant=12.8 是 VisDrone 经验值
        nwd_val = nwd_loss(pred_bboxes, gt_bboxes, constant=12.8)

        # 3. 融合 (GIoU + NWD)
        #    这样 NWD 自动继承 GIoU 的权重，不用改 YAML
        combined_loss = giou_val + (1 - nwd_val)

        # 4. 赋值
        loss[name_giou] = combined_loss.sum() / len(gt_bboxes)
        # === 🟢 W-RT-DETR 修改结束 ===
